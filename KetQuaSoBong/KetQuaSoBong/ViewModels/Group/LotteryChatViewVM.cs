...
{
    public class LotteryChatViewVM : BindableBase
    {
        ...
        private LotteryChatService _lotteryChatService;
        public LotteryChatViewVM(ContentView contentView, LotteryChatService footballChatService)
        {
            this._lotteryChatService = footballChatService;
            ...
            SendCommand = new Command(async () =>
            {
                ...
                if (ContentChat != "")
                {
                    bool b = await _lotteryChatService.SendChatAsync(chat);
                    if (b == true)
                    {
                        ItemChats.Add(chat);
                        GetAllChatAsync(contentView);
        ...
        {
            List<ItemChat> chats = await _lotteryChatService.GetAllChat();
            if (chats.Count > 0)
            {
